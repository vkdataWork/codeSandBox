<template>
  ...
  <DxDashboardControl ref="dashboard" class="responsive-paddings" :endpoint="apiUrl" style="height: 90dvh"
    ...
    :onDashboardEndUpdate="(e: any) => onDashboardEndUpdate(e)" :default-extensions="[DxExtensions.DataSourceWizard]"
    :extensions="[
    ...
    <DxExtensions :enableCustomSql="true">
      ...
      <DxDashboardParameterDialog :allowParameterEditing="true" :allowParameterAdding="true"
        ...
        :allowParameterTypes="['System.String', 'System.DateTime', 'System.Int32', 'System.Int64', 'System.Double', 'System.Boolean']" />
...
onBeforeMount(async () => {
  document
    ...
    .setAttribute('href', `${location.protocol}//${location.hostname}:${location.port}/css/analytics/dx-analytics.material.teal.dark.compact.css`);
  document
    .getElementById('themeDashboard')!
    .setAttribute('href', `${location.protocol}//${location.hostname}:${location.port}/css/dashboard/dx-dashboard.material.teal.dark.compact.css`);

  workingModeProp.value = route.params.mode === 'designer' || route.params.mode == 'designercopy' ? 'Designer' : 'Viewer';
