...
export class PolarChartItemExtension implements ICustomItemExtension {
	...
	public createViewerItem = (model: any, element: any, content: any) => {
		return new PolarChartItem(model, element, content, this.dashboardControl);
	};
}
...
export class PolarChartItem extends CustomItemViewer {
	...
	constructor(model: any, container: any, options: any, dashboardControl: dxDashboardControl) {
		...
		this.polarChartContainer = this.dashboardControl.dashboardContainer();
	}

	destroy() {
		this.dxPolarWidget?.dispose();
	}
		...
		if (series) {
			for (let i = 0; i < series.length; i++) {
				...
				for (let j = 0; j < points.length; j++) {
					if (this.isSelected(points[j].data.clientDataRow)) points[j].select();
					else points[j].clearSelection();
				}
			}
		}
	}
