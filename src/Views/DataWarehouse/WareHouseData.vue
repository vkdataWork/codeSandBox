    ...
    <div class="scroll-wrapper">
        <div v-if="columns && columnsType.length > 0" :key="count">
            <DxDataGrid ref="dataGrid" :key="render" class="datagrid responsive-paddings" :data-source="storeDataGrid"
                ...
                :focused-row-enabled="false" :remote-operations="{ groupPaging: true }" :word-wrap-enabled="true"
                :onEditorPreparing="onEditorPreparing" :onSaving="onSaving"
                :onEditingStart="(e: any) => onEditingStart(e)">
                ...
                <DxColumn v-for="(column, index) in columns" :key="index" :data-field="column" :caption="column"
                    ...
                    <DxAsyncRule :validation-callback="(e: any) => asyncValidation(e, column)"
                        message="Input is invalid" v-if="isCustomColumn(column)" />
                </DxColumn>

                <template #cidr="{ data }">
                    <cidr-box :values="data" :isCidr="true"
                        @update:cidr="(e: any, values: any) => onCidrChanged(e, values)" />
                </template>
                <template #inet="{ data }">
                    <cidr-box :values="data" :isCidr="false"
                        @update:inet="(e: any, values: any) => onInetChanged(e, values)" />
                </template>
                <template #macaddress="{ data }">
                    <mac-address-box :values="data"
                        @update:macaddress="(e: any, value: any) => onMacAddressChanged(e, value)"
                ...
                <template #interval="{ data }">
                    <interval-box :values="data"
                        @update:interval="(e: any, value: any) => onIntervalChanged(e, value)" />
                </template>
                <template #bytea="{ data }">

                    <div v-if="data.value">
                        <div style="display: flex; flex-direction: row" v-if="!hideImage">
                            <img v-if="data.value" :src="getImage(data.value)" alt="No image"
                ...
                <template #byteaGrid="{ data }">
                    <img v-if="data.value" :src="getImage(data.value)" alt="Picture"
